...
/*Görev 1: macSkoru()
  ...
  skor1 sırayla artarken skor2 hep 1 değerini verecektir.
  2. Hangisi bir closure kullanmaktadır? Nasıl tarif edebilirsin? (yarınki derste öğreneceksin :) )
  closure skor1'de kullanılmaktadır. Closure genellikle return'de function yazıldığında yapılır.
...
function macSonucu(takimSkoru, toplamCeyrek) {
  let takims = {
    EvSahibi: takimSkoru() * toplamCeyrek,
    KonukTakim: takimSkoru() * toplamCeyrek,
  };
...
function periyotSkoru(takimSkoru) {
  let takims = { EvSahibi: takimSkoru(), KonukTakim: takimSkoru() };
...
function skorTabelasi(cbPeriod, cbScore, q) {
  ...
  let scoreBoard=[];
  for (let i=0;i<q;i++){
    const periodResult=periyotSkoru(takimSkoru);
...
console.log(skorTabelasi(periyotSkoru, takimSkoru, 4) + "son");
